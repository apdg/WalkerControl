...
float targetKilometers = 2;
float estimatedMpS = 1.3;
...
// 10.3k from 8m ms, 6.8 from 5.5m - looking like 1.2 is reliable on black phone - not anymore. 1.1 and .96 recently. back to 1.2571698, did 2k at 1.38 20%. Just moved motor mount lower and repeating > 2.6km, so 1.56! seeing if that repeats... nope set to 5k, got 3.7 
//hmm, it actually stopped itself at 2549 seconds, so it still managed nearly 1.5 mps,  but why did it interrupt? Code knows it ended  But it didn't end on the timer  ended as if button was pressed.
